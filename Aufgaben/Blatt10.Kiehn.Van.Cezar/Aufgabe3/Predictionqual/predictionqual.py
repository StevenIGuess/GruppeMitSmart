...
def parse_command_line(argv):
  p=argparse.ArgumentParser(description=('Input the gold standard followed by the prediction files'))
...
def evaluate(gs,prediction):
    intersect=gs & prediction
    return (len(intersect),len(intersect)/len(gs),len(intersect)/len(prediction))
...
def main():
    ...
    for inputfile in args.inputfiles:
        ...
        print(f"{inputfile}\t{eval[0]}\t{100*eval[1]:.2f}\t{100*eval[2]:.2f}\t{100*mean:.2f}")
